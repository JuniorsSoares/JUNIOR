...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      // Se não tiver perfil ou estiver errado, faz upsert no banco
      ...
      if (isOwnerEmail && (!profileData || profileData.role !== 'admin')) {
        ...
        await supabase.from('user_profiles').upsert({
          id: corrected.id,
          full_name: corrected.full_name,
          email: corrected.email,
          unit_name: corrected.unit_name,
          role: corrected.role,
        });
        profile = corrected;
      } else if (profileData) {
      ...
      // Filtra por unidade se for professor
      const isAdmin = profile.role === 'admin';

      const studentsQuery = isAdmin
        ? supabase.from('students').select('*').order('name')
        : supabase.from('students').select('*').eq('unit_name', profile.unit_name).order('name');

      const entriesQuery = isAdmin
        ? supabase.from('log_entries').select('*').order('date', { ascending: false })
        : supabase.from('log_entries').select('*').eq('unit_name', profile.unit_name).order('date', { ascending: false });
